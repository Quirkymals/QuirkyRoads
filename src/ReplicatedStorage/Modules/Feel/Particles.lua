...
function Particles.CreateParticle(key: string | {})
	...
	-- if not self.CanCreateParticle then return end

	-- self.CanCreateParticle = false
...
function Particles.Connect(key: string | {})
	...
	local CurrentlyMoving: boolean = false
	...
	self.Connections["MoveDirection"] = Humanoid:GetPropertyChangedSignal("MoveDirection"):Connect(function()
		...
		Active = true
		while self.Connections and self.Connections["MoveDirection"] do
			if
